		...
		<td>
			...
			{foreach from=$submission->getReviewAssignments() item=reviewAssignments}
				{foreach from=$reviewAssignments item=assignment name=assignmentList}
					...
					<tr valign="top">
						<td width="33%" style="padding: 0 4px 0 0; font-size: 1.0em">{if $assignment->getDateNotified()}{showdate value=$assignment->getDateNotified() format=$dateFormatTrunc}{else}&mdash;{/if}</td>
						...
						<td width="34%" style="padding: 0 4px 0 0; font-size: 1.0em">{if $assignment->getDateCompleted()}{showdate value=$assignment->getDateCompleted() format=$dateFormatTrunc}{else}&mdash;{/if}</td>
					</tr>
					{/if}
				{foreachelse}
				<tr valign="top">
			...
			{foreach from=$submission->getDecisions() item=decisions}
				{foreach from=$decisions item=decision name=decisionList}
					{if $smarty.foreach.decisionList.last}
							{showdate value=$decision.dateDecided format=$dateFormatTrunc}
					{/if}
				{foreachelse}
					&mdash;
				{/foreach}
