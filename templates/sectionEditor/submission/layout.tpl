...
<table width="100%" class="info">
	...
	<tr>
		...
		<td>
			{if $useLayoutEditors}
				...
				{/if}
				{showdate value=$layoutAssignment->getDateNotified() format=$dateFormatShort default=""}
			{else}
		...
		<td>
			{if $useLayoutEditors}
				{showdate value=$layoutAssignment->getDateUnderway() format=$dateFormatShort}
			{else}
				{translate key="common.notApplicableShort"}
			{/if}
		</td>
		<td>
			{if $useLayoutEditors}
				{showdate value=$layoutAssignment->getDateCompleted() format=$dateFormatShort}
			{else}
				{translate key="common.notApplicableShort"}
			{/if}
		</td>
		<td colspan="2">
			{if $useLayoutEditors}
				...
				{else}
					{icon name="mail" disabled="disable"}
				{/if}
				{showdate value=$layoutAssignment->getDateAcknowledged() format=$dateFormatShort default=""}
			{else}
				{translate key="common.notApplicableShort"}
			{/if}
		</td>
	...
	<tr valign="top">
		<td colspan="6">
			...
			{if $layoutFile}
				<a href="{url op="downloadFile" path=$submission->getArticleId()|to_array:$layoutFile->getFileId()}" class="file">{$layoutFile->getFileName()|escape}</a>&nbsp;&nbsp;{showdate value=$layoutFile->getDateModified() format=$dateFormatShort}
			{else}
	...
	<tr>
		...
		<td width="26%">{$galley->getGalleyLabel()|escape} &nbsp; <a href="{url op="proofGalley" path=$submission->getArticleId()|to_array:$galley->getGalleyId()}" class="action">{translate key="submission.layout.viewProof"}</a></td>
		<td colspan="2"><a href="{url op="downloadFile" path=$submission->getArticleId()|to_array:$galley->getFileId()}" class="file">{$galley->getFileName()|escape}</a>&nbsp;&nbsp;{showdate value=$galley->getDateModified() format=$dateFormatShort}</td>
	...
	<tr>
		...
		<td colspan="2"><a href="{url op="downloadFile" path=$submission->getArticleId()|to_array:$suppFile->getFileId()}" class="file">{$suppFile->getFileName()|escape}</a>&nbsp;&nbsp;{showdate value=$suppFile->getDateModified() format=$dateFormatShort}</td>
...
{if $submission->getMostRecentLayoutComment()}
	...
	<a href="javascript:openComments('{url op="viewLayoutComments" path=$submission->getArticleId() anchor=$comment->getCommentId()}');" class="icon">{icon name="comment"}</a>{showdate value=$comment->getDatePosted() format=$dateFormatShort}
{else}
