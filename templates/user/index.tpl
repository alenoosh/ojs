...
<ul class="plain">
	...
	{if $journalPaymentsEnabled && $subscriptionEnabled && $userHasSubscription}
		<li>&#187; <a href="{url page="user" op="payRenewSubscription"}">{translate key="payment.subscription.renew"}</a> ({translate key="payment.subscription.expires"}: {showdate value=$subscriptionEndDate format=$dateFormatShort})</li>
	{/if}
	{if $journalPaymentsEnabled && $membershipEnabled}
		{if $dateEndMembership}
			<li>&#187; <a href="{url page="user" op="payMembership"}">{translate key="payment.membership.renewMembership"}</a> ({translate key="payment.membership.ends"}: {showdate value=$dateEndMembership format=$dateFormatShort})</li>
		{else}
