...
{if !$showToc && $issue}
	...
	<br />
	{if $coverPagePath}<div id="issueCoverImage"><a href="{$currentUrl}"><img src="{$coverPagePath|escape}"{if $coverPageAltText != ''} alt="{$coverPageAltText|escape}" title="{$coverPageAltText|escape}"{else} alt="{translate key="issue.coverPage.altText"}" title="{translate key="issue.coverPage.altText"}"{/if}{if $width} width="{$width|escape}"{/if}{if $height} height="{$height|escape}"{/if}/></a></div>{/if}
