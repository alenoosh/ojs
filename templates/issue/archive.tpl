...
{iterate from=issues item=issue}
	...
	{if $issue->getFileName($locale) && $issue->getShowCoverPage($locale)}
		<div class="issueCoverImage"><a href="{url op="view" path=$issue->getBestIssueId($currentJournal)}"><img src="{$coverPagePath|escape}{$issue->getFileName($locale)|escape}"{if $issue->getCoverPageAltText($locale) != ''} alt="{$issue->getCoverPageAltText($locale)|escape}" title="{$issue->getCoverPageAltText($locale)|escape}"{else} alt="{translate key="issue.coverPage.altText"}" title="{translate key="issue.coverPage.altText"}"{/if}/></a>
		</div>
