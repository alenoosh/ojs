...
{if $currentJournal->getSetting('submissionFeeEnabled')}
	...
	{if $submissionPayment}
		<td width="80%" colspan="2">{translate key="payment.paid"} {showdate value=$submissionPayment->getTimestamp() format=$datetimeFormatLong}</td>
	{else}
...
{if $currentJournal->getSetting('fastTrackFeeEnabled')}
	...
	{if $fastTrackPayment}
		<td width="80%" colspan="2">{translate key="payment.paid"} {showdate value=$fastTrackPayment->getTimestamp() format=$datetimeFormatLong}</td>
	{else}
...
{if $currentJournal->getSetting('publicationFeeEnabled')}
	...
	{if $publicationPayment}
		<td width="80%" colspan="2">{translate key="payment.paid"} {showdate value=$publicationPayment->getTimestamp() format=$datetimeFormatLong}</td>
	{else}
