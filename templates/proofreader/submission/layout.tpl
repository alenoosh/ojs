...
<table width="100%" class="info">
	...
	<tr>
		...
		<td width="35%">{$galley->getGalleyLabel()} &nbsp; <a href="{url op="proofGalley" path=$submission->getArticleId()|to_array:$galley->getGalleyId()}" class="action">{translate key="submission.layout.viewProof"}</td>
		<td><a href="{url op="downloadFile" path=$submission->getArticleId()|to_array:$galley->getFileId()}" class="file">{$galley->getFileName()|escape}</a> {showdate value=$galley->getDateModified() format=$dateFormatShort}</td>
	</tr>
	{foreachelse}
	<tr>
	...
	<tr>
		...
		<td><a href="{url op="downloadFile" path=$submission->getArticleId()|to_array:$suppFile->getFileId()}" class="file">{$suppFile->getFileName()|escape}</a> {showdate value=$suppFile->getDateModified() format=$dateFormatShort}</td>
	</tr>
	{foreachelse}
	<tr>
		<td colspan="3" class="nodata">{translate key="common.none"}</td>
