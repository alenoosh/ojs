...
class UserExportDom {
	function &exportUsers(&$journal, &$users, $allowedRoles = null) {
		...
		foreach ($users as $user) {
			...
			// Opatan Inc.
			if (is_array($user->getFirstName(null))) {
				foreach($user->getFirstName(null) as $locale => $value) {
					$firstNameNode =& XMLCustomWriter::createChildWithText($doc, $userNode, 'first_name', $value, false);
					if ($firstNameNode) {
			...
			// Opatan Inc.
			if (is_array($user->getMiddleName(null))) {
				foreach($user->getMiddleName(null) as $locale => $value) {
					$middleNameNode =& XMLCustomWriter::createChildWithText($doc, $userNode, 'middle_name', $value, false);
					if ($middleNameNode) {
